...
commit `b8f7021`). Only the headers differ, apart from `ssm.diffmat`'s error identifiers.
Both libraries use the lower Cholesky factor throughout; bvar-toolkit switched
`ksc_rw_h0` to it in `d3b9494`, with bitwise the same draws.
