...
| `ssm.select_obs` | the split of stacked data into observed and missing values, `y = So*yo + Sm*ym`, written from Section 2.1 of Chan, Poon and Zhu (2023), whose code is not archived here | unit (`test_select_obs`), 19 September 2026: both illustrations of that section reproduced exactly; `[So, Sm]` a permutation matrix; the conditional mean and precision of the missing values of a VAR(1) with a hole and a ragged edge equal to dense Gaussian conditioning |
