...
% (1) Anchors: each anchor script in tests/fixtures/, run whole on a short chain,
%     stores the same draws to within 1e-10 of their scale, and leaves the random
%     number stream in the same state, when its posterior mean (computed with K\c)
%     and its inline draw of the states are replaced by one call to
%     ssm.simulate_states, which computes the mean from the Cholesky factor. The
%     anchors are sp_code's UC.m (the published code of Chan and Jeliazkov 2009) and
%     chan-jeliazkov-2009's UC.m, linreg_tvp.m and DFM.m. Each patch must match
%     exactly one line.
% (2) Moments: on a simulated local level model, the mean returned equals K\b and
%     the Kalman smoother's mean, inv(K) equals the smoother's covariances, and
%     20,000 draws match its means, variances and lag-one covariances.
...
A(1) = struct('dir', sp, 'files', {{'UC.m', 'SVRW.m', 'USCPI.csv'}}, 'script', 'UC.m', 'seed', 1, ...
    ...
    'swap', {{'Ctau = chol(Ktau,''lower'');', ''; ...
              'tauhat = Ktau\(invSigy*y);', '    [tau, tauhat] = ssm.simulate_states(Ktau, invSigy*y);'; ...
...
A(2) = struct('dir', cj, 'files', {{'UC.m', 'USCPI.csv'}}, 'script', 'UC.m', 'seed', [], ...
    ...
    'swap', {{'tau_hat = Ktau\(tau0/omega2*HHiota + y/sig2);', ...
                  '    [tau, tau_hat] = ssm.simulate_states(Ktau, tau0/omega2*HHiota + y/sig2);'; ...
...
A(3) = struct('dir', cj, 'files', {{'linreg_tvp.m', 'SURform.m', 'USPCE_OutputGap.csv'}}, 'script', 'linreg_tvp.m', 'seed', [], ...
    ...
    'swap', {{'CKbeta = chol(Kbeta, ''lower'');', ''; ...
              'beta_hat = Kbeta\(P*repmat(beta0,T,1) + Zy/sig2);', ...
                  '    [beta, beta_hat] = ssm.simulate_states(Kbeta, P*repmat(beta0,T,1) + Zy/sig2);'; ...
...
A(4) = struct('dir', cj, 'files', {{'DFM.m', 'FRED-MD.csv'}}, 'script', 'DFM.m', 'seed', [], ...
    ...
    'swap', {{'f_hat = Kf \ (kron(speye(T), A'' * iSig) * y);', ...
                  '    [f, f_hat] = ssm.simulate_states(Kf, kron(speye(T), A'' * iSig) * y);'; ...
              'f = f_hat + chol(Kf,''lower'')'' \ randn(T*r,1);', ''}}, ...
    'out', {{'store_F', 'store_A', 'store_sig2', 'store_omega2', 'store_phi'}});

tol = 1e-10;                                   % relative to the scale of the draws
for ii = 1:numel(A)
    a = A(ii);
    leg = run_anchor(a, a.run, sprintf('legacy%d', ii));
    new = run_anchor(a, [a.run; a.swap], sprintf('ssm%d', ii));
    assert(isequal(leg.rngstate, new.rngstate), ...
        'simulate_states: %s leaves the random number stream in another state', ...
        fullfile(a.dir, a.script));
    for v = a.out
        L = leg.(v{1}); N = new.(v{1});
        assert(max(abs(L(:) - N(:))) <= tol*max(1, max(abs(L(:)))), ...
...
nd = 20000;
rng(12, 'twister');
[X, tauhat] = ssm.simulate_states(K, b, nd);
assert(max(abs(tauhat - K\b)) < 1e-12 && max(abs(tauhat - m_ks)) < 1e-10, ...
    'simulate_states: the mean differs from K\\b or from the smoother mean');
