% This script estimates the marginal likelihood of the SV-t model.
% See:
%
...
function [ml mlstd] = ml_sv_t(y,store_theta,hhat,prior,M)
...
theta_IS(:,5) = tnormrnd(thetahat(5),thetavar(5),2,100,M);

phih_const = 1/(normcdf(1,thetahat(3),thetastd(3))-normcdf(-1,thetahat(3),thetastd(3)));
nu_const = 1/(normcdf(100,thetahat(5),thetastd(5))-normcdf(2,thetahat(5),thetastd(5)));
gIS = @(m,mh,ph,oh,n) -.5*log(2*pi*thetavar(1)) -.5*(m-thetahat(1))^2/thetavar(1)...
    ...
    -.5*log(2*pi*thetavar(5)) + log(nu_const) -.5*(n-thetahat(5))^2/thetavar(5);
for loop = 1:M
    theta = theta_IS(loop,:)';
    mu = theta(1);
    muh = theta(2);
    ...
    nu = theta(5);
    s2 = (y-mu).^2;     
    llike = intlike_sv_t(s2,muh,phih,omegah2,nu,hhat,50); 
    store_w(loop) = llike + prior(mu,muh,phih,omegah2,nu) ...
        - gIS(mu,muh,phih,omegah2,nu);    
end
