% This script estimates the marginal likelihood of the GARCH-MA model.
% See:
%
...
function [ml mlstd] = ml_garch_ma(y,store_theta,prior,sig20,M)
M = 20*ceil(M/20);
store_gam = log(store_theta(:,3:end));
muhat = mean(store_theta(:,1))';
mupre = 1/var(store_theta(:,1));
mustd = sqrt(var(store_theta(:,1)));
psihat = mean(store_theta(:,2))';
psipre = 1/var(store_theta(:,2));
psistd = sqrt(var(store_theta(:,2)));
...
theta_IS(:,2) = tnormrnd(psihat,1/psipre,-.999,.999,M);
theta_IS(:,3:end) = repmat(gamhat',M,1) + (gamstd*randn(3,M))';

store_w = zeros(M,1);
psi_const = 1/(normcdf(1,psihat,psistd)-normcdf(-1,psihat,psistd));
gIS = @(m,p,g) -.5*log(2*pi/mupre) -.5*(m-muhat)^2*mupre ...
    +log(psi_const) -.5*log(2*pi/psipre) -.5*(p-psihat)^2*psipre ... 
...
for loop = 1:M
    ...
    if sum(exp(gam(2:3)))<1  % check the draw is stationary        
        llike = loglike_garch_ma(y-mu,psi,gam,sig20);    
        store_w(loop) = llike + prior(mu,psi,gam) - gIS(mu,psi,gam);
    else
        store_w(loop) = -10^100;
    end
end
