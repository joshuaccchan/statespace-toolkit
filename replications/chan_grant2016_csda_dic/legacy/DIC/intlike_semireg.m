function llike = intlike_semireg(ymXbeta,G,D,sig2,tau,V1,V2)

% This function evaluates the log-integrated likelihood of the
...
[n m] = size(D);
c = -n/2*log(2*pi) -.5*log(V1) -.5*log(V2);
invOmega = sparse(1:m,1:m,[1/V1 1/V2 1/tau*ones(1,m-2)]);
GinvOmegaG = G'*invOmega*G;
Ktheta = D'*D/sig2 + GinvOmegaG;
dtheta = D'*ymXbeta/sig2;
C = chol(Ktheta,'lower');
llike = c - n/2*log(sig2) + sum(log(diag(G))) - (m-2)/2*log(tau) ...
    - sum(log(diag(C))) -.5*((ymXbeta'*ymXbeta)/sig2 - dtheta'*(Ktheta\dtheta));
end
