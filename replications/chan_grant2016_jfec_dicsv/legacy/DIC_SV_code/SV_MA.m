...
for loop = 1:nloop
    ...
    if loop>burnin
        ...
        store_theta((bigloop-1)*(nloop-burnin)+i,:) =  [mu psi muh phih omegah2];  
        if  DIC_choice == 1 % observed-data DIC        
            llike = intlike_sv_ma(y-mu,psi,muh,phih,omegah2,ht,50);
            lpost = llike + prior(mu,psi,muh,phih,omegah2);       
        elseif DIC_choice == 2; % conditional DIC        
            llike = conlike_sv_ma(y-mu,h,psi);
            lpost = like_sv_ma(y-mu,h,psi,muh,phih,omegah2) ... 
                + prior(mu,psi,muh,phih,omegah2);
        end
        store_llike(i,:) = llike; 
        store_lpost(i,:) = lpost;        
    end
