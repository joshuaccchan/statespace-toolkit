...
store_theta = zeros(R*(nloop - burnin),4); % [mu muh phih omegah2]
...
for loop = 1:nloop 
    ...
    if loop>burnin
        ...
        store_theta((bigloop-1)*(nloop-burnin)+i,:) =  [mu muh phih omegah2];        
        s2 = (y-mu).^2;        
        if  DIC_choice == 1 % observed-data DIC
            llike = intlike_sv(s2,muh,phih,omegah2,ht,50);
            lpost = llike + prior(mu,muh,phih,omegah2);
        elseif DIC_choice == 2; % conditional DIC
            llike = conlike_sv(s2,h);
            lpost = like_sv(s2,h,muh,phih,omegah2) + prior(mu,muh,phih,omegah2);
        end        
        store_llike(i,:) = llike; 
        store_lpost(i,:) = lpost;
    end    
