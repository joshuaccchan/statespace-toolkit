% This script estimates the deterministic trend model 
%
% See:
...
phi_const = 1/(count/R);
prior = @(m,ph,sc,ta0) -.5*log(2*pi*Vmu) -.5*(m-mu0)^2/Vmu ...
    -log(2*pi)+.5*log(det(invVphi))+log(phi_const)-.5*(ph-phi0)'*invVphi*(ph-phi0)...
    + pri_sigc2(sc) -.5*log(2*pi*Vtau0) - .5*(ta0-tau00)^2/Vtau0;

disp('Starting MCMC for DT.... ');
disp(' ' );
start_time = clock; 
...
store_theta = zeros(nsims,5); % [mu, phi, sigc2, tau0]
...
for isim = 1:nsims+burnin
    ...
    Kdel = diag([1/Vtau0  1/Vmu]) + Xdel'*(Hphi'*Hphi)*Xdel/sigc2;
    delhat = Kdel\([tau00/Vtau0; mu0/Vmu] + 1/sigc2*Xdel'*(Hphi'*Hphi)*y);
...
if cp_ml
    ...
    [ml,mlstd] = ml_DT(y,store_theta,prior,M);
