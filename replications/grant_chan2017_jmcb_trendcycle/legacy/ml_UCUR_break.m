...
% correlated errors and one break
% 
% See:
...
function [ml mlstd] = ml_UCUR_break(y,store_theta,t0,prior,M)
M = 20*ceil(M/20);
...
rhohat = mean(store_theta(:,7));
rhovar = var(store_theta(:,7));
tau0hat = mean(store_theta(:,8));
tau0var = var(store_theta(:,8));
...
theta_IS(:,6) = 1./gamrnd(nusigtau2hat,1./Ssigtau2hat,M,1);
theta_IS(:,7) = tnormrnd(rhohat,rhovar,-.999,.999,M);
theta_IS(:,8) = tau0hat + sqrt(tau0var)*randn(M,1);

store_w = zeros(M,1);

rho_const = 1/(normcdf(1,rhohat,sqrt(rhovar))-normcdf(-1,rhohat,sqrt(rhovar)));
gIS = @(m,ph,sy,st,r,ta0) -log(2*pi)-.5*sum(log(muvar)) -.5*sum((m-muhat).^2./muvar) ...
...
for isim = 1:M    
    ...
    if sum(phi) < .99 && phi(2) - phi(1) < .99 && phi(2) > -.99
        llike = intlike_UCUR_break(y,mu,phi,sigc2,sigtau2,rho,tau0,t0);
