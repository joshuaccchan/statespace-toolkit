% This script estimates the UCSV-RV-h model in Chan and Song (2018)
% 
% See:
...
ma =  [0; .5; .5]; %coefficients of x equation
...
Prior.Vystar = 10 * exp(-Prior.mg); %variance of y1* is Vystar * exp(g1); variance of y1 is var(y1*) + exp(h1)
Prior.mystar = 5;

Prior.Vh = 10; %variance of h1
Prior.mh = 1; %mean of h

Prior.Ka = eye(3); %precision of coefficients of the logz equation
Prior.ma = ma;
...
%% MCMC
rng(123);
Post = sy_CS3(Data, Prior, Ini, MCMC);
