...
function Post = sy_CS3(Data, Prior, Ini, MCMC)
    %% data
    y = Data.y;
    z = Data.z;
    logz = log(z);
    ...
    h = Ini.h; % h equation (y log volatility)
    g = Ini.g; % g equation (ystar log volatility)
    ...
    for iter = 1 : Nsim
        ...
        ghat = Kg\(Omegas_inv * (ytilde-ds) + [mg/Vg; zeros(T-1, 1)] + a(2)/sigma2z * (logz - a(1) - a(3) * h));
        P = chol(Kg);
        g = ghat + P\randn(T, 1);
        ...
        Kh = Omegaz_inv + H'*Sv_inv*H + a(3)^2 / sigma2z * IT;
        hhat = Kh\(Omegaz_inv * (ytilde - dz) + [mh/Vh; zeros(T-1, 1)] + a(3) / sigma2z * (logz - a(1) - a(2) * g));
        P = chol(Kh);
        h = hhat + P\randn(T, 1);
        ...
        szhat = sz + sum((logz-a(1)-a(2)*g -a(3)*h).^2); % logz equation (RV measurement): error variance
