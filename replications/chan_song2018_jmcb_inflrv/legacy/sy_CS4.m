...
% log(z) also has SV
function Post = sy_CS4(Data, Prior, Ini, MCMC)
    %% data
    y = Data.y;
    z = Data.z;
    logz = log(z);
    ...
    sigma2v = Ini.sigma2v; % v equation (log(z) log volatility): error variance
    ystar = Ini.ystar; % ystar values
    g = Ini.g; % h equation (y log volatility)
    h = Ini.h; % g equation (ystar log volatility)
    v = Ini.v; % v equation (log(z) log volatility)
    ...
    vv = Prior.vv; % v equation (log(z) log volatility): error variance
    sv = Prior.sv; % v equation (log(z) log volatility): error variance
    ...
    Vv = Prior.Vv; % variance of v1
    mv = Prior.mv; % mean of v1
    ...
    for iter = 1 : Nsim
        ...
        Sigmaz_inv = spdiags(exp(-v), 0, T, T);
        Kg = Omegas_inv + H' * Sv_inv * H + a(2)^2 * Sigmaz_inv;
        ghat = Kg\(Omegas_inv * (ytilde-ds) + [mg/Vg; zeros(T-1, 1)] + a(2) * Sigmaz_inv * (logz - a(1)));
        P = chol(Kg);
        g = ghat + P\randn(T, 1);
        ...
        % Zv: mixture indicator
        ztilde =log((logz - a(1) - a(2) * g).^2);
        PS = cumsum(KSC_W .* normpdf(repmat(ztilde - v, 1, 7), KSC_M, KSC_V),2);
        Zv =  1 + sum(repmat(rand(T,1) .* PS(:, 7), 1, 7 ) > PS, 2);

        % v: log volatility of log(z) equation
        ds = KSC_m(Zv);
        Omegav_inv = spdiags(1./KSC_v2(Zv), 0, T, T);
        Sv_inv =  spdiags([1/Vv, repmat(1/sigma2v, 1, T-1)]', 0, T, T);
        Kv = Omegav_inv + H' * Sv_inv * H;
        vhat = Kv \ (Omegav_inv * (ztilde - ds) + [mv/Vv; zeros(T-1, 1)]);
        P = chol(Kv);
        v = vhat + P\randn(T, 1);
        
        %sigma2h, sigma2g, sigma2v
        ...
        svhat = sv + sum(diff(v).^2); % v equation (log(z) log volatility): error variance
        vvhat = vv + T - 1; % v equation (log(z) log volatility): error variance
    
        sigma2h = 1/gamrnd(vhhat/2, 2/shhat);
        sigma2g = 1/gamrnd(vghat/2, 2/sghat);
        sigma2v = 1/gamrnd(vvhat/2, 2/svhat);
    
        % a
        Sigmaz_inv = spdiags(exp(-v), 0, T, T);
        X(:,2) = g;
        Kahat = Ka + X' * Sigmaz_inv * X;
        mahat = Kahat\(Ka * ma + X'* Sigmaz_inv * logz);
        ...
        if iter > Burnin
            ...
            Post.expv_pm= Post.expv_pm + exp(v/2);
            Post.v_pm= Post.v_pm + v;
        end
    end
    ...
    Post.expv_pm = Post.expv_pm / Nuse;
    Post.v_pm = Post.v_pm / Nuse;
